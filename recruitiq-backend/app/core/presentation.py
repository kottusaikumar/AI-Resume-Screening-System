...
def compute_match_label(match_percentage: float) -> str:
    if match_percentage >= 75:
        return "High Evidence Alignment"
    if match_percentage >= 50:
        return "Moderate Evidence Alignment"
    if match_percentage >= 30:
        return "Low Evidence Alignment"
...
def compute_retention_risk(
    ...
    """Compatibility field: retention is not inferred from resume proxies."""
    return "Not assessed"
...
def generate_alignment_summary(
    ...
    strength = compute_match_label(match_percentage).replace(" Evidence Alignment", "").lower()
    ...
    return (
        f"This resume shows {strength} evidence alignment "
        f"(alignment index {match_percentage:.0f}/100) with the job description, "
...
def compute_confidence(
    ...
    """
    # TF-IDF is diagnostic-only because it overlaps with BM25.
    scores = [dense_score, bm25_score, keyword_coverage]
