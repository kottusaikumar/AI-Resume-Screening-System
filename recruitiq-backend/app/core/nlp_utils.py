...
try:
    if os.getenv("USE_SPACY", "false").strip().lower() not in {"1", "true", "yes", "on"}:
        raise ImportError("spaCy disabled; set USE_SPACY=true to enable it")
    import spacy
    ...
    logging.getLogger("recruitiq").warning(
        "spaCy unavailable; using deterministic taxonomy fallback: %s", error
    )
...
def extract_skills_from_text(text: str) -> List[str]:
    ...
    if nlp is None or _phrase_matcher is None:
        found: dict[str, str] = {}
        for alias, canonical in SKILL_LOOKUP.items():
            pattern = rf"(?<![A-Za-z0-9]){re.escape(alias)}(?![A-Za-z0-9])"
            if re.search(pattern, cleaned, flags=re.IGNORECASE):
                found.setdefault(normalise(canonical), canonical)
