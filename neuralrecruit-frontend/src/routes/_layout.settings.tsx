...
import { rebalanceWeights } from "@/lib/scoring-weights";
...
const WEIGHT_META: { key: keyof ScoringWeights; label: string; desc: string }[] = [
  ...
  {
    key: "positional_skill",
    label: "Positional Skill Score",
    desc: "Bonus for skills in high-value sections (Summary, Skills).",
  },
  {
    key: "experience_skill",
    label: "Experience-Skill Score",
    desc: "Correlation between skills and work history duration.",
  },
  {
    key: "resume_quality",
    label: "Resume Quality",
    desc: "Holistic score for ATS-friendliness and presentation.",
  },
];
...
function SettingsPage() {
  ...
  const total = weights
    ? weights.dense +
      ...
      weights.tfidf +
      weights.keyword +
      weights.positional_skill +
      weights.experience_skill +
      weights.resume_quality
    : 0;

  const updateWeight = (key: keyof ScoringWeights, value: number) => {
    setWeights((current) => (current ? rebalanceWeights(current, key, value) : current));
    setSaveState("idle");
  };
      ...
      {status === "loading" ? (
        ...
        <section className="glass rounded-xl p-6 md:p-8">
          ...
          <p className="text-xs text-muted-foreground mt-2">
            Changing one weight automatically redistributes the others so the total always remains
            100%.
          </p>
